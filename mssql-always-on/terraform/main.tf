...
resource "nebius_vpc_network" "ya-network" {
    name = "ya-network"
}

resource "nebius_vpc_subnet" "ya-sqlserver-rc1a" {
  name       = "ya-sqlserver-rc1a"
  zone       = var.zone-a
  network_id = "${nebius_vpc_network.ya-network.id}"
  v4_cidr_blocks = ["192.168.1.0/28"]
}

resource "nebius_vpc_subnet" "ya-sqlserver-rc1b" {
...
resource "nebius_vpc_subnet" "ya-sqlserver-rc1c" {
...
resource "nebius_vpc_subnet" "ya-ilb-rc1a" {
  name       = "ya-ilb-rc1a"
  zone       = var.zone-a
  network_id = "${nebius_vpc_network.ya-network.id}"
...
resource "nebius_vpc_subnet" "ya-ad-rc1a" {
  name       = "ya-ad-rc1a"
  zone       = var.zone-a
  network_id = "${nebius_vpc_network.ya-network.id}"
...
resource "nebius_lb_target_group" "ya-tg" {
  name      = "ya-tg"
  region_id = var.region_id

  target {
    subnet_id = "${nebius_vpc_subnet.ya-sqlserver-rc1a.id}"
    address   = var.vm-ya-mssql1-internal-ipv4
  }

  target {
    subnet_id = "${nebius_vpc_subnet.ya-sqlserver-rc1b.id}"
    address   = var.vm-ya-mssql2-internal-ipv4
  }

  target {
    subnet_id = "${nebius_vpc_subnet.ya-sqlserver-rc1c.id}"
    address   = var.vm-ya-mssql3-internal-ipv4
  }
}

/*creating Network Load Balancer*/
resource "nebius_lb_network_load_balancer" "ya-loadbalancer" {
  ...
  listener {
    ...
    internal_address_spec {
      ip_version = "ipv4"
      subnet_id = nebius_vpc_subnet.ya-ilb-rc1a.id
    }
  }

  attached_target_group {
    target_group_id = "${nebius_lb_target_group.ya-tg.id}"
...
/*creating VM ya-jump1 - Bastion Host witn NAT*/
 resource "nebius_compute_instance" "ya-jump1" {
  name        = "ya-jump1"
  hostname    = "ya-jump1"
  ...
  network_interface {
    subnet_id = nebius_vpc_subnet.ya-ad-rc1a.id
    nat       = true
  }
...
/*creating VM ya-ad - Active Directory Server*/
 resource "nebius_compute_instance" "ya-ad" {
  name        = "ya-ad"
  hostname    = "ya-ad"
  platform_id = var.platform_id
  zone        = var.zone-a
...
resource "nebius_compute_disk" "ya-mssql1-db-disk" {
 ...
 resource "nebius_compute_instance" "ya-mssql1" {
  name        = "ya-mssql1"
  hostname    = "ya-mssql1"
  platform_id = var.platform_id
  zone        = var.zone-a
  ...
  secondary_disk {
    disk_id = nebius_compute_disk.ya-mssql1-db-disk.id
    mode = "READ_WRITE"
    auto_delete = true
  }
...
resource "nebius_compute_disk" "ya-mssql2-db-disk" {
 ...
 resource "nebius_compute_instance" "ya-mssql2" {
  name        = "ya-mssql2"
  hostname    = "ya-mssql2"
  platform_id = var.platform_id
  zone        = var.zone-b
  ...
  secondary_disk {
    disk_id = nebius_compute_disk.ya-mssql2-db-disk.id
    mode = "READ_WRITE"
    auto_delete = true
  }
...
resource "nebius_compute_disk" "ya-mssql3-db-disk" {
 ...
 resource "nebius_compute_instance" "ya-mssql3" {
  name        = "ya-mssql3"
  hostname    = "ya-mssql3"
  platform_id = var.platform_id
  zone        = var.zone-c
  ...
  secondary_disk {
    disk_id = nebius_compute_disk.ya-mssql3-db-disk.id
    mode = "READ_WRITE"
    auto_delete = true
  }
